...
class CustomerController{
    static placeOrder = async (req, res)=> {
        ...
        for(let storeOrder of ordersDetails) {
            //form order entry
            //find store entry
            let store = await StoreModel.findById(storeOrder.storeId).lean().select("userId");
            if(!store)
                return res.status(400).json("No store entry found");
            
            let order = new OrderModel({customerId:req.user._id,storeId:storeOrder.storeId, vendorId:store.userId, address:{coordinates:[storeOrder.address.latitude, storeOrder.address.longitude]}});
            //calculate the total price of order and push it to pending process
            let orderTotalPrice  = 0;
            for(const orderItem of storeOrder.items) {
                //find sizedItem
                let orderItemCost = 0;
                let sizedItem = await ItemSizesModel.findById(orderItem.itemSizeId).select("price").lean();
                if(!sizedItem)
                    return res.status(400).json({message: "Cant find entry with such id"});
                ...
                pendingProcess.push(new OrderItemModel({attributes:orderItem.addons, quantity:orderItem.quantity, itemSizeId:orderItem.itemSizeId, orderId: order._id, orderCost:orderItemCost}).save());
            }
            //push order
