...
class DispatcherController {
    ...
    static acceptOrRejectOrder = async (req, res) => {
        ...
        if(details.status.toUpperCase() === "ACCEPTED") {
            ...
            await new OrderRiderStatusModel({orderId:order._id, riderId:req.user._id}).save();
            ...
            let virtualAccountRider = await VirtualAccountModel.findOne({id:req.user._id});
            let virtualAccountStore = await VirtualAccountModel.findOne({id:order.storeId})
            virtualAccountRider.amount += (order.deliveryCost * 0.75)
            virtualAccountStore.amount += order.itemCost
            Promise.all(orderRider.save(), order.save(), virtualAccountRider.save(), virtualAccountStore.save())
        }
    ...
    static operationArea = async (req, res) => {
        ...
        try {
            ...
            //find rider
            let riderDetails = await riderDetailsModel.findOne({userId:req.user._id});
            if(riderDetails) {
                riderDetails.operationArea = details.location;
                riderDetails.save()
            } else {
                await new riderDetailsModel({operationArea:details.location, userId:req.user._id}).save()
            }
