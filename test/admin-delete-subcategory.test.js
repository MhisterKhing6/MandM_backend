...
describe("test codes for delete subcategories", () => {
  let user = "";
  let user1 = {
    role: "customer",
    name: "kingsley",
  ...
  let subCategory = new subCategoriesModel({categoryId:category._id, name:"samSung"})
  ...
  it("should return status of 200", async () => {
    let response = await request(app)
      .delete(`${url}/${subCategory._id}`)
      .set("Accept", "application/json")
      .set("Authorization", `Bearer ${token2}`)
      .set("content-type", "application/json");
    let sub = await subCategoriesModel.findById(subCategory._id)
    assert.isNull(sub)
